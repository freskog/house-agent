...
class Transcriber:
    ...
    def initialize(self) -> bool:
        ...
        try:
            with self.lock:
                ...
                logger.info("🛠️ Setting C locale to avoid Whisper segmentation fault...")
                try:
                ...
                logger.info(f"📥 Loading transcription model: {self.config.model_name}")
            ...
            logger.error(f"❌ Error initializing transcription model: {e}")
            return False
    ...
    def _apply_audio_gain_optimization(self, audio_np: np.ndarray) -> tuple[np.ndarray, float]:
        ...
        if 10.0 < audio_rms < 200.0:
            ...
            logger.debug(f"🔊 Amplified quiet audio: gain={gain_factor:.1f}x, RMS {audio_rms:.2f} -> {new_rms:.2f}")
    ...
    def transcribe_audio(self, audio_data: bytes, sample_rate: int = 16000, thread_id: Optional[str] = None) -> TranscriptionResult:
        ...
        try:
            ...
            if self.model is None:
                ...
                logger.info("🎤 Transcription model initialized on demand")
            ...
            if audio_rms < 100:  # Arbitrary threshold, adjust as needed
                logger.warning(f"🔇 Audio too quiet (RMS: {audio_rms:.2f}), using fallback")
                return TranscriptionResult(
            ...
            logger.info(f"🎧 Transcribing {len(audio_data)/1024:.1f}KB audio ({duration:.2f}s, RMS: {audio_rms:.2f})")
            
            # Validate audio data
            if len(audio_data) < 1000:
                logger.warning(f"⚠️ Audio data very short ({len(audio_data)} bytes)")
            ...
            logger.debug(f"🍎 CoreML setting: WHISPER_COREML={coreml_setting}")
            ...
            logger.info(f"🚀 Starting whisper transcription with in-memory processing...")
            ...
            if not text.strip():
                logger.warning(f"🔇 Empty transcription after {process_time:.2f}s, using fallback")
                return TranscriptionResult(
            ...
            logger.info(f"✅ Transcription completed in {process_time:.2f}s (gain: {gain_factor:.1f}x)")
            return result
            ...
            logger.info("🔄 Attempting fallback to file-based transcription...")
    ...
    def _transcribe_fallback_file_method(self, audio_data: bytes, sample_rate: int, thread_id: Optional[str]) -> TranscriptionResult:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(suffix='.wav', delete=True) as temp_file:
                ...
                logger.info(f"Fallback: Using temporary file {temp_path}")
